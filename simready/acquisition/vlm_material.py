...
All dimensions provided to you are strictly in standard METERS (m), and volume is in CUBIC METERS (m³).
You MUST conduct all your density and mass calculations using standard SI units (kg/m³ for density, kg for mass).
Do NOT mix millimeters and meters. When writing your `reasoning_step`, explicitly state the dimensions in meters \
to prove you understand the scale before concluding the material and mass.

CRITICAL PHYSICAL CONTEXT:
You are provided with the exact bounding box dimensions (in meters) and the volume of the object.
- Use the dimensions to judge the scale! A 0.04m part is a small ~4 cm component (e.g. pipe fitting, small fastener). A 0.20m part is a mid-size industrial component. A 2.0m part is large infrastructure.
...
INSTRUCTIONS:
1. Analyze the Semantic Label, part name, and bounding box dimensions (in meters) to understand the real-world scale and application.
2. Write a brief `reasoning_step` that explicitly states the dimensions in meters, then concludes the most likely material.
3. Because you have exact dimensions, BE CONFIDENT. For standard industrial parts, your `confidence` should be > 0.85. Only use low confidence (< 0.8) if the part is physically ambiguous.
...
def classify_material_vlm(
    ...
    if bbox_m is not None:
        bbox_str = f"{bbox_m[0]:.4f}m (X) x {bbox_m[1]:.4f}m (Y) x {bbox_m[2]:.4f}m (Z)"
    else:
        bbox_str = "unknown"

    if volume_m3 is not None:
        volume_str = f"{volume_m3:.6f} cubic meters"
    else:
        volume_str = "unknown"
